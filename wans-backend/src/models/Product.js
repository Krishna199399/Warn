...
const productSchema = new mongoose.Schema({
  ...
  actualPrice: { type: Number, required: true, min: 0 },  // Base price before discount (excluding tax)
  price:       { type: Number, required: true, min: 0 },  // Selling price (after discount, excluding tax)
...
productSchema.pre('save', async function () {
  ...
  if (this.actualPrice && this.wholesalePrice) {
    this.wholesaleMargin = this.actualPrice - this.wholesalePrice;
  }
  if (this.actualPrice && this.miniStockPrice) {
    this.miniStockMargin = this.actualPrice - this.miniStockPrice;
  }
